...
DatasetName = {'COVID_7d';'COVID_14d';'COVID_30';'COVID7_WINDOWX2';'COVID7_WINDOWX3';'COVID7_WINDOWX4';'COVID7_WINDOWX5'};
% OptimizerName = {'MLP_PSO';'MLP_GWO';'FSA'};
OptimizerName = {'PSO';'GWO';'FSA';'IGWO';'ACO';'GBO';'HPSOGWO';'JSOA';'SMA';'WOA'};

%% Parameters Configuration %%
RunNo=1;                                   % Max Run
SearchAgentsNo=60;                         % Number of search agents
MaxIteration=50;                           % Maximum number of iterations / SearchAgents
Max_nfe= round(MaxIteration*SearchAgentsNo);
...
for DatasetNo = 7:7
    ...
    for OpimizerNo = 3:3%1:10%1:size(OptimizerName,1)
        ...
        for HiddenNode =18:18%19:19
            ...
            parfor run = 1:RunNo
                ...
                if OpimizerNo == 3
                    [BestScore(run,:),BestPosition(run,:),ConvergenceCurve3(run,:)] = FSA(SearchAgentsNo,Max_nfe,lb,ub,dim,fobj,mlpConfig);
                end
                if OpimizerNo == 4
...
%display(ClassificationRate(3,3));
%figure()
%semilogy(ConvergenceCurve3,'r')
%drawnow

%display('--------------------------------------------------------------------------------------------')
%display('Classification rate')
%display('  MLP_PSO    MLP_GWO     FSA     MLP_HPSOGWO   MLP_GHPSOGWO   MLP_ADAMHPSOGWO   MLP_FODHPSOGWO')
%display(mean(ClassificationRate(:,end),2))
%display('--------------------------------------------------------------------------------------------')
...
%semilogy(mean(ConvergenceCurve3,1),'bo')
%emilogy(mean(ConvergenceCurve4,1),'y')
%emilogy(mean(ConvergenceCurve5,1),'g')
%semilogy(mean(ConvergenceCurve6,1),'c')
%semilogy(mean(ConvergenceCurve7,1),'g-')
%semilogy(mean(ConvergenceCurve8,1),'c*')
%semilogy(mean(ConvergenceCurve9,1),'r-')
%semilogy(mean(ConvergenceCurve10,1),'b-')
%xlabel('Generation');
%ylabel('MSE');%
...
%grid on
%box on
%legend('PSO','GWO','FSA','IGWO','ACO','GBO','HPSOGWO','JSOA','SMA','WOA')%

%Draw classification rates
%subplot(1,2,2);
%hold on
%title('Classification Accuracies')
%bar(mean(ClassificationRate(:,end),2))
%xlabel('Algorithm');
%ylabel('Classification rate (%)');

%grid on
%box on
%set(gca,'XTickLabel',{'PSO','GWO','FSA','IGWO','ACO','GBO','HPSOGWO','JSOA','SMA','WOA'});
