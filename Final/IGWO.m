...
function [Alpha_score,Alpha_pos,Convergence_curve]=IGWO(N,Max_iter,lb,ub,dim,fobj,mlpConfig) % SearchAgents_no,Max_iter,lb,ub,dim,fobj,mlpConfig
disp('I-GWO is now tackling your problem');
...
for i=1:size(Positions,1)
    Fit(i) = fobj(Positions(i,:),mlpConfig);
end
...
while iter < Max_iter
    ...
    for i=1:size(Positions,1)
        ...
        Fit_GWO(i) = fobj(X_GWO(i,:),mlpConfig);
    end
    ...
    for t=1:N
        ...
        Fit_DLH(t) = fobj(X_DLH(t,:),mlpConfig);
    end
